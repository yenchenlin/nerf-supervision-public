...
def load_colmap_data(realdir):
    ...
    keys_map = {i:j for j, i in enumerate(imdata.keys())}
    ...
    return poses, pts3d, perm, keys_map


def save_poses(basedir, poses, pts3d, perm, keys_map):
    pts_arr = []
    vis_arr = []

    for k in pts3d:
        cams = [0] * (poses.shape[-1] )
        image_ids = [keys_map[id] for id in pts3d[k].image_ids if id in keys_map]
    ...
    # print(pts_arr.shape, valid_z.shape, perm)
...
def gen_poses(basedir, match_type, factors=None):
    ...
    poses, pts3d, perm, keys_map = load_colmap_data(basedir)
    
    save_poses(basedir, poses, pts3d, perm, keys_map)
